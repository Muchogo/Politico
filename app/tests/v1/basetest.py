from unittest import TestCase
from ... import create_app
from flask import json
app = create_app()


class BaseTestCase(TestCase):

    def setUp(self):

        self.app = app.test_client()
        self.app.testing = True

        # USER TEST DATA
        
        user_signup_data = {"first_name": "Eric",
                            "last_name": "Muchogo",
                            "other_names": "Ndungu",
                            "phonenumber": "0725530122",
                            "email": "ericmuchogooooo@gmail.com",
                            "username": "Valerieee",
                            "password": "1000"}
        duplicate_username_signup_data = {"first_name": "Eric",
                                          "last_name": "Muchogo",
                                          "other_names": "Ndungu",
                                          "phonenumber": "0725530122",
                                          "email": "ericmuchogo2@gmail.com",
                                          "username": "Muchogo2",
                                          "password": "1000"}
        duplicate_email_signup_data = {"first_name": "Eric",
                                       "last_name": "Muchogo",
                                       "other_names": "Ndungu",
                                       "phonenumber": "0725530122",
                                       "email": "ericmuchogo5@gmail.com",
                                       "username": "Muchogo3",
                                       "password": "1000"}
        duplicate_email = {
            "first_name": "Eric",
            "last_name": "Muchogo",
            "other_names": "Ndungu",
            "phonenumber": "0725530122",
            "email": "ericmuchogo5@gmail.com",
            "username": "Muchogo10",
            "password": "1000"
        }
        wrong_signup_data = {"first_name": "Eric",
                             "last_name": "Muchogo",
                             "other_names": "Ndungu",
                             "email": "ericmuchogo@gmail.com",
                             "username": "Muchogo2",
                             "password": "1000"}
        admin_signup_data = {"first_name": "Eric",
                             "last_name": "Muchogo",
                             "other_names": "Ndungu",
                             "phonenumber": "0725530122",
                             "email": "ericmuchogo@gmail.com",
                             "username": "Master",
                             "password": "1000",
                             "isAdmin": True}
        login_data = {"username": "Muchogo", "password": "1000"}
        nonexisting_user = {"username": "James", "password": "1000"}
        wrong_login_data = {"username": "Muchogo", "password": "68990"}
        wrong_login_data_format = {
            "username_name": "Muchogo", "password": "1000"}
        self.user_data = json.dumps(user_signup_data)
        self.admin_data = json.dumps(admin_signup_data)
        self.login_data = json.dumps(login_data)
        self.wrong_login_data = json.dumps(wrong_login_data)
        self.wrong_signup_data = json.dumps(wrong_signup_data)
        self.duplicate_username_signup_data = json.dumps(
            duplicate_username_signup_data)
        self.duplicate_email_signup_data = json.dumps(
            duplicate_email_signup_data
        )
        self.email = json.dumps(duplicate_email)
        self.wrong_login_data_format = json.dumps(wrong_login_data_format)
        self.nonexisting_user = json.dumps(nonexisting_user)

        # PARTIES TEST DATA

        image_in_base64 = "data:image/png;base64,"
        new_parties_data = {
            "manifesto": "Peace",
            "headquaters": "nairobi",
            "createdBy": 1,
            "images": [image_in_base64, image_in_base64],
            "videos": [image_in_base64, image_in_base64]
        }
        new_parties_data_nonexisting_user = {
            "manifesto": "Peace",
            "headquaters": "nairobi",
            "createdBy": 100,
            "images": [image_in_base64, image_in_base64],
            "videos": [image_in_base64, image_in_base64]
        }
        new_parties_data_with_wrong_format = {
            "manifesto": "Peace",
            "headquaters": "nairobi",
            "createdBy": 1,
            "images": [image_in_base64, image_in_base64],
            "videos": [image_in_base64, image_in_base64]
        }
        self.new_parties_data = json.dumps(new_parties_data)
        self.new_parties_data_with_wrong_format = json.dumps(
            new_parties_data_with_wrong_format)
        self.new_parties_data_nonexisting_user = json.dumps(
            new_parties_data_nonexisting_user)



        # ASPIRANTS TEST DATA

        image_in_base64 = "data:image/png;base64,"
        new_aspirants_data = {
            "memorandum": "amani",
            "parties": "ODM",
            "createdBy": 1,
            "images": [image_in_base64, image_in_base64],
            "videos": [image_in_base64, image_in_base64]
        }
        new_aspirants_data_nonexisting_user = {
            "memorandum": "upendo",
            "parties": "ODM",
            "createdBy": 100,
            "images": [image_in_base64, image_in_base64],
            "videos": [image_in_base64, image_in_base64]
        }
        new_aspirants_data_with_wrong_format = {
            "memorandum": "upendo",
            "parties": "ODM",
            "createdBy": 1,
            "images": [image_in_base64, image_in_base64],
            "videos": [image_in_base64, image_in_base64]
        }
        self.new_aspirants = json.dumps(new_aspirants_data)
        self.new_aspirants_data_with_wrong_format = json.dumps(
            new_aspirants_data_with_wrong_format)
        self.new_aspirants_data_nonexisting_user = json.dumps(
            new_aspirants_data_nonexisting_user)

    def get_req(self, url):
        return self.app.get(url)
